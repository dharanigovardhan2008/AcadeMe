        ...
        <GlassCard>
            ...
            <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', color: 'white' }}>
                    <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}><th style={{textAlign:'left', padding:'1rem'}}>S.No</th><th style={{textAlign:'left'}}>User</th><th style={{textAlign:'left'}}>Reg No</th><th style={{textAlign:'left'}}>Branch</th><th style={{textAlign:'left'}}>Status</th><th style={{textAlign:'right'}}>Actions</th></tr></thead>
                    <tbody>
                        {users.map((user, index) => (
                            <tr key={user.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                <td style={{ padding: '1rem' }}><div style={{width:'30px', height:'30px', background:'rgba(255,255,255,0.1)', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center'}}>{index + 1}</div></td>
...
const FacultyManagement = () => {
    ...
    const [newFaculty, setNewFaculty] = useState({ name: '', designation: '', phone: '', department: 'CSE', courses: [] });
    ...
    const handleAddFaculty = async (e) => {
        ...
        try {
            ...
            setNewFaculty({ name: '', designation: '', phone: '', department: 'CSE', courses: [] }); 
            alert("Faculty added!"); fetchFaculty();
        } catch (error) { console.error(error); }
        setLoading(false);
    };
        ...
        <GlassCard>
            ...
            <form onSubmit={postUpdate} style={{display:'grid', gap:'1rem', marginBottom:'2rem'}}>
                ...
                <div style={{position:'relative'}}>
                    ...
                    <input type="url" placeholder="Link (Optional)" value={newUpdate.link} onChange={e=>setNewUpdate({...newUpdate, link:e.target.value})} style={{width:'100%', padding:'10px 10px 10px 35px', background:'rgba(0,0,0,0.2)', border:'1px solid rgba(255,255,255,0.1)', color:'white', borderRadius:'8px'}} />
                </div>
                <GlassButton type="submit" variant="gradient">Post Update</GlassButton>
            </form>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>{updates.map(u => (<div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px', background: 'rgba(255,255,255,0.05)', borderRadius: '8px' }}><div><div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem' }}><h4 style={{ fontWeight: 'bold' }}>{u.title}</h4><span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{new Date(u.date).toLocaleDateString()}</span></div><p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.8)' }}>{u.message}</p>{u.link && <a href={u.link} target="_blank" rel="noreferrer" style={{ fontSize: '0.85rem', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '5px', marginTop: '5px' }}><ExternalLink size={14} /> View Resource</a>}</div><GlassButton onClick={() => handleDeleteUpdate(u.id)} style={{ padding: '8px', background: 'rgba(255, 255, 255, 0.1)' }}><Trash2 size={16} /></GlassButton></div>))}</div>
        </GlassCard>
    );
};
            ...
            {activeTab === 'overview' && (
                <>
                    ...
                    <GlassCard>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {recentUsers.length === 0 ? <p style={{ color: 'var(--text-secondary)' }}>No recent signups.</p> : recentUsers.map(user => (
